        ...
        uint2* __restrict__ ranges)
{
    ...
    bool curr_valid = (currtile != (uint32_t) -1);
    ...
    if (idx == 0) {
        ...
        uint32_t prevtile   = static_cast<uint32_t>(prev_key >> 32);
        bool prev_valid     = (prevtile != (uint32_t) -1);
...
std::tuple<int,int> CudaRasterizer::Rasterizer::forward(
	...
	bool antialiasing,
	int* radii,
	float* alphaLeft, // PART: jwq's modification
	bool debug)
{
	...
	// NOTE: jwq: directly copy the accum_alpha to alphaLeft. imgState.accum_alpha is not used in FW of this code, maybe used in BW.
...
void CudaRasterizer::Rasterizer::backward(
	...
	float* dL_dscale,
	float* dL_drot,
	const float* colors_bg, // PART: jwq's modification
	bool antialiasing,
	bool debug)
{
	...
	CHECK_CUDA(BACKWARD::render(
		...
		dL_dcolor,
		dL_dinvdepth,
		colors_bg), debug) // PART: jwq's modification
