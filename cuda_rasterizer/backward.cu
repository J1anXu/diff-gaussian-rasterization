...
PerGaussianRenderCUDA(
	...
	const float* __restrict__ colors_bg) // PART: jwq's modification
{
	...
	// PART: we also thread shuffle the bg color in the backward pass
	float bg[C] = {0.0f};
	...
	const float* colors_bg) // PART: jwq's modification
{
	...
	PerGaussianRenderCUDA<NUM_CHANNELS_3DGS> <<<((B*32) + THREADS - 1) / THREADS,THREADS>>>(
		...
		dL_dcolors, 
		dL_dinvdepths,
		colors_bg); // PART: jwq's modification
}
