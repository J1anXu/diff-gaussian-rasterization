	...
	const bool debug)
{
  ...
  // PART: jwq's modification
  torch::Tensor alphaLeft = torch::full({H, W}, 1.0, float_opts); // NOTE: initiate a high level container from PyTorch.
  int rendered = 0;
  int num_buckets = 0;
  if(P != 0)
  {
	  ...
	  auto tup = CudaRasterizer::Rasterizer::forward(
		...
		alphaLeft.contiguous().data<float>(), // PART: jwq's modification
		debug);
		
		rendered = std::get<0>(tup);
		num_buckets = std::get<1>(tup);
  }
  return std::make_tuple(rendered, num_buckets, out_color, out_invdepth, radii, geomBuffer, binningBuffer, imgBuffer, sampleBuffer, alphaLeft);
}
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
 RasterizeGaussiansBackwardCUDA(
	...
	const torch::Tensor& colors_bg, // PART: jwq's modification
	const bool antialiasing,
	const bool debug) 
{
  const int P = means3D.size(0);
  ...
  {  
	  ...
	  colors_bg.contiguous().data<float>(), // PART: jwq's modification
	  antialiasing,
	  debug);
  }
